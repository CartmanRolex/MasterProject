...
| `plot_orange_outcomes.py` | `figures/orange_outcome_standard.pdf` (standard fine-tuning — action expert + state projection only — grouped by source; §4.1 Figure 1) **and** `figures/orange_outcome_variants.pdf` (fine-tuning variants: standard vs LM-tuned vs fully-tuned vs no-tail, custom families only; the fully-tuned bar — `Gal-pick-orange-tailedCH20-unfrozen-all`, 58%/2.32, best in the comparison, 40k steps vs 20k — exists for Teleop subtask only; §4.1 Figure 2, placed after the variants paragraph). Pure-Python via `plot_lib`. |
