...
| `so101_quest3.py` | Meta Quest 3 hand-tracking teleop device. Reuses Gregorio's WebXR server (aiohttp page streamed to the Quest's Meta Browser) and emits an 8D delta `[dx, 0, dz, drvx, drvy, drvz, d_shoulder_pan, d_gripper]`; thumb–index pinch closes the gripper. **Left-hand clutch:** closing the *left* hand into a fist freezes all commands (the robot holds still) so the operator can recentre a right hand that has drifted (translation is incremental, so an offset accumulates); opening it resumes, re-anchored from the right hand's new pose with no jump — the same path as a brief tracking loss. A left hand not tracked at all reads as open (never freezes). Detected by `left_hand_closed` (`quest3_webxr.py`) from the left-hand joints — a finger is curled when its tip is within `left_fist_curl_ratio`× its knuckle's wrist-distance, fist when ≥3 of 4 fingers curl; the WebXR page streams `left_joints` alongside the right hand. **Two rotation aids:** keyboard **L** toggles a *forward-rotation (roll) lock* — `_on_keyboard_event` flips `_freeze_forward_rot`, and `_anchored_rotation_error` then projects the gripper-forward axis (body −z) out of the error so the roll holds at its current value with no jump (same trick as the up axis); and `right_rot_in_deg` / `right_rot_out_deg` (CLI `--quest_right_rot_in_deg` / `--quest_right_rot_out_deg`, wired in `teleop_se3_agent.py`) **symmetrically remap** the *right-axis* rotation only (the `w_root[1]` pitch component before the swap): a real wrist pitch of `in_deg` reaches `out_deg` on the gripper (e.g. 70→90, since 90° is hard to reach), the same on both signs (`np.clip` + sign-preserving scale) and saturating beyond ±`in_deg`; `in_deg ≤ 0` = off. Like `so101_gamepad_v3`, **lateral (left/right) hand motion drives the `shoulder_pan` joint directly** (a bounded relative target, tuned by `shoulder_pan_sensitivity`) and `shoulder_pan` is **removed from the IK** — the DLS-IK (over the remaining 4 joints: shoulder_lift, elbow_flex, wrist_flex, wrist_roll) handles forward/back + up/down + wrist rotation. The root-Y (lateral) component drives `d_shoulder_pan` (rotating the arm), while **forward/back + up/down are commanded in the gripper body frame** and rotated into the root frame the IK consumes — the same thing `gamepad_v3` does via `_convert_delta_from_frame` (gripper-frame axis convention: forward=−z, up=+x, left/right=y→shoulder_pan). This is required because, with `shoulder_pan` removed, the 4 IK joints can only translate the gripper within the vertical plane at the current `shoulder_pan` azimuth; a fixed root-+X forward command leaves that plane as soon as the arm is panned and stalls (up/down, being in every vertical plane, kept working). The gripper frame moves with the arm so forward/up stay reachable and translation follows where the gripper points. `anchor_rot` (the hand-body re-expression in `xr_delta_to_world`) is therefore passed `None` for quest3 — the gripper-frame application provides the orientation-following, and doing both would double-rotate. Forward/back is also negated (it came out mirrored). Imports its WebXR page / state from `quest3_webxr.py`. **Position commands are decomposed in the HAND's own body frame** (from the live wrist quaternion, `rot_now.inv().apply(wrist_pos − prev)`), exactly the axes the monitor's FWD/RIGHT/UP triad shows (XR hand axes: +X=right, +Y=up, +Z=back ⇒ forward=−back). So a command is *how far the hand moved along its own forward/right/up axes*, not a component of the wrist position in fixed room axes — moving the hand forward always means "forward where the hand points". The matrix `_R_XR_TO_ISAAC_SO101` (`[[0,0,-1],[-1,0,0],[0,1,0]]`) is now used **only** for the rotation map (`_R_rot_map = Rotation.from_matrix(...)`), not for position. This matrix is **also** reused as `_R_rot_map` (`Rotation.from_matrix`), which needs a proper rotation (det +1), so a mirrored *translation* axis must NOT be fixed by flipping a row here (that makes det −1 and scipy rejects it); forward/back came out mirrored and is instead negated on the root-frame output (`out[0] = -dpos_root[0]`) which it passes to `xr_delta_to_world` as `R`; `get_device_state` then rotates that world delta into the robot base frame with `_world_to_root`, so the relative DLS-IK term gets a true root-frame delta and the robot moves up / right / back exactly as the monitor and `so101_gamepad_v3` name those axes. This is the world-frame companion of the monitor's display matrix (`C @ M_monitor`), so device and monitor stay consistent while serving different frames; the device's copy is independent so the monitor is unaffected. **Position is incremental, rotation is absolute+anchored** (mixed mode achieved with no action-layer change): position stays a per-step delta re-expressed in the hand's **current body frame** — rotated by the wrist orientation since an anchor (`_anchor_wrist_rot`, captured on start/reset/tracking-loss and held until the next reset) before `R` maps it, so "move hand forward" always means "move the gripper forward along where it points"; rotation is **absolute** — each step the device feeds the unchanged 6D relative-mode IK the **root-frame error rotvec** from the current gripper orientation to a target = the gripper's anchor orientation (`_anchor_ee_world_rot`) composed with the hand's rotation-since-anchor (mapped via `_R_rot_map`). The IK's relative mode applies that as a world/root-frame left-multiplication (`ee_quat_des = dQ @ ee_quat`), so the gripper chases the hand's absolute orientation every step — drift-free, no per-step delta accumulation, no sign/swap hacks; `max_rot_step_rad` is the rate cap (`rot_scale` no longer applies to rotation). The anchor (hand + gripper orientation) is held until the next reset, so the gripper mirrors the hand's rotation from its start pose with no start jump and any constant hand↔gripper orientation offset auto-cancels. If a rotation axis comes out swapped/wrong, `_R_rot_map` (defaults to the same matrix as position; may be split into a dedicated one) is the single tuning knob. Axis fixes live in `_anchored_rotation_error` (root-frame axes `[forward, right, up]`): the hand's **forward↔right rotation is swapped on the commanded target (feedforward)** — rotating the hand about its forward axis was turning the gripper about the right axis; the right-axis rotation is also sign-inverted (it came out reversed); doing the swap on the closed-loop *error* instead makes the IK chase a permuted direction and diverge, so it is applied to the command and the error stays honest/stable. Three corrections (**all verified in a numerical simulation** of `_anchored_rotation_error`) remove the cross-axis bleed seen on hardware — one on the hand side, two on the gripper side: **(1) the hand's heading is stripped (gimbal-free swing–twist about vertical)** before the command is read. The command is the axis-angle of the hand's rotation-since-anchor, and the axis-angle of a compound rotation does not separate cleanly, so a 90° hand *yaw* rotates a forward *roll* into the right axis (visible in the monitor as the "right" value moving while the hand only rolls about the middle-finger axis); the swing–twist split removes the heading twist (owned by `shoulder_pan`), leaving pitch+roll. **(2) The chase target is re-headed to the gripper's current heading** (rotated by the pan-since-anchor `dpsi` about root up; `dpsi` = azimuth of the gripper forward axis now minus at the anchor) — left-multiplying the root-frame command onto the anchor leaves a large heading error in the chase rotvec once `shoulder_pan` pans the arm, which bleeds a forward hand rotation into the gripper's right axis. **(3) The gripper's own up axis is freed, not root up** — heading is owned by `shoulder_pan`, so the gripper-up component (body +x) is projected out of the error; when the wrist is pitched, gripper-up tilts away from root up, so the old `zero root-Z` dropped the wrong component and bled a forward roll into pitch. In simulation a single-axis hand rotation (roll/pitch) then produces a single constant-axis gripper rotation at every hand yaw and gripper pan/pitch, and a hand yaw produces none; the naive versions only looked correct at the start where all frames coincide. The anchor's gripper orientation is captured as the current EE on the same first frame as the wrist anchor, so the hand rotation reference is synchronized with the robot at frame 1 (zero error, no start jump). Because the 4-joint DLS-IK weights position and rotation equally and the same joints serve both, hard orientation tracking starves translation; `rot_track_gain` (ctor arg, default 0.4, CLI `--quest_rot_track_gain`) scales the rotation error down so orientation gives way to translation (still converges when you stop translating; 1.0 = track orientation hard). `fwd_up_gain` (ctor arg, default 1.0, CLI `--quest_fwd_up_gain`) is an extra multiplier applied only to the gripper-frame `up_cmd`/`fwd_cmd` (forward/back, up/down) after `pos_scale` and the `max_pos_step_m` clamp — raise it to make those two axes travel faster per metre of hand motion without changing left/right pan speed (tuned separately by `shoulder_pan_sensitivity`). Lives at `devices/so101_quest3.py` (not under `gamepad/`). |
| `quest3_webxr.py` | Shared, Isaac-free WebXR layer for Quest 3 teleop: the WebXR page (`_HTML_TEMPLATE`), the `_R_XR_TO_ISAAC` frame matrix, `_TeleopState`, the pinch indices, `left_hand_closed` (left-hand fist detector for the freeze clutch), and the `xr_delta_to_world` per-step delta math (used by the device). `xr_delta_to_world` takes an optional `anchor_rot`: when given (by the device, not the monitor) it re-expresses the room-frame position delta in the hand's current body frame — rotated by the wrist orientation since the anchor — before `R` maps it, so the translation follows the hand's pointing direction; when `None` (the monitor, which does not call it) the legacy room-frame mapping is used unchanged. `_R_XR_TO_ISAAC` maps the XR wrist axes `[right, up, back]` into the **monitor's** robot display frame `[up, right, back]` (idx0=up, idx1=right with positive=to-the-right, idx2=back, forward=−idx2); it is a reflection (det −1) used only for the calibration monitor's display. The live device passes its own `_R_XR_TO_ISAAC_SO101` to `xr_delta_to_world`, so the device and monitor axis maps are independent. Imported by **both** `so101_quest3.py` (in the leisaac tree) and `quest3_hand_monitor.py`. Lives at `devices/quest3_webxr.py`. |
| `quest3_hand_monitor.py` | Standalone Quest 3 **orientation/frame calibration tool** (no Isaac, no GPU). Runs the same WebXR server and serves a self-contained browser dashboard (`/monitor`) with **one wrist-centered 3D view** (drag to orbit) that plots the right-hand skeleton plus the wrist body-frame **triad labelled FWD / RIGHT / UP** (body +X=RIGHT, +Y=UP, +Z=BACK ⇒ FWD=−Z), a motion trail and a room-frame gizmo. The side panel reports **position** (fwd/right/up, m) from a position origin anchored on connect, **rotation from level** (about fwd/right/up, deg) and the **raw wrist quaternion** (absolute + since-level), plus pinch distance and gripper state. The rotation reference is the hand's **heading at reset, leveled** (forward/right forced into the horizontal plane, pitch & roll removed), so a flat hand reads zero and rotation shows tilt from horizontal. The live hand's heading is stripped each frame (gimbal-free swing-twist about vertical) before the fwd/right values are read, so a forward roll reads as pure *about fwd* at any hand yaw — matching the live device's `_anchored_rotation_error` de-yaw, so the monitor predicts the robot's rotation. Reset origin re-anchors the position origin and re-levels the rotation reference to the current heading. Used to verify the XR→robot axis mapping visually — if a triad arrow does not line up with the physical hand, the axis assignment is wrong — without launching the sim. Uses the monitor's own `_R_XR_TO_ISAAC` (in `quest3_webxr.py`), which is independent of the device's `_R_XR_TO_ISAAC_SO101`. MasterProject-only — not deployed into the leisaac tree. |
...
visible while you rotate it. The side panel reports **position** (fwd / right / up,
m) measured from a position origin anchored on connect, **rotation from level**
(about fwd / right / up, deg) and the **raw wrist quaternion** (w, x, y, z) both
absolute and since-level — plus pinch vs threshold and an OPEN/CLOSED gripper. The
rotation reference is the hand's **heading at reset, leveled** — its forward and
right axes forced into the horizontal plane (pitch & roll removed, up = room +Y) —
so a flat hand reads zero and the readout shows tilt away from horizontal. The live
hand's heading is also stripped each frame (gimbal-free swing-twist about vertical)
before the fwd/right values are read, so a forward roll reads as pure *about fwd* at
any hand yaw — the same de-yaw the live device applies in `_anchored_rotation_error`,
so the monitor predicts the robot's rotation. A
**Reset origin** button re-anchors the position origin to the current pose and
re-levels the rotation reference to the current heading (also done automatically
when the Quest reconnects).
