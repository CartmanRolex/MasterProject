...
| `compute_place_success.py` | (no figure) Prints the place success rate for the orchestrated subtask models (`tab:place_success`), anchored to the geometric `oranges_in_plate` count over genuine place attempts (success + drop + subtask-timeout), and decomposes genuine failures into drops vs. subtask-timeouts. Corrects the gripper-retraction confirmation gate that logs placed-but-unretracted oranges as timeouts. Reads checkpoints under `isaac-inference/results`; pure stdlib. |
