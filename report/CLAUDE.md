...
**Note (results rewrite).** The results section is now: `fig:failure_modes` (grasp-bottleneck composition, restored — `plot_failure_modes.py`, GRASP shown strict force-confirmed ~37%, not the geometric override), `tab:grasp_chain` + `tab:redirection` (both fed by `compute_grasp_chain.py`: grasp success, secured/place-once-secured with per-formulation "secured" = force-grasp for subtask / lift for monotask, Level-1 clean-vs-recovered, Level-2 redirection = GRASP-timeout→different-orange, distinct oranges tried, empty-episode rate), and the obedience analysis. The monotask "tried"/redirection cells are a deliberate `pending` placeholder (needs per-step proximity). The old lift/place-by-scene-state/recovery/failure-geometry tables were removed, so `compute_recovery.py`, the by-scene-state columns of `compute_place_success.py`, and the geometry/timeout parts of `compute_failure_modes.py` no longer feed report tables (its `composition` helper is still used by `plot_failure_modes.py` for the LIFT/PLACE bars).
