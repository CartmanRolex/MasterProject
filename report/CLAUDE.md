...
- `results.tex` — draft; `\section{Discussion}` with `\subsection{Conclusion}`, `\subsection{Limitations}`, then `\subsection{Future Work}` (forward-looking content consolidated in Future Work: grasp policy, 6-DOF arm, harder tasks, VLM-based success judging / replacing the FSM)
