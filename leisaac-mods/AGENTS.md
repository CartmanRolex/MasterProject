...
matrix. Rotation is corrected in `get_device_state`, not the matrix: the X↔Y swap
line negates the component that lands on the left/right axis (EE pitch, which is
inverted on this headset) — if the wrong rotation axis ends up inverted, move
the minus sign to the other swapped term. Translation is **wrist-orientation-aware**: `get_device_state` passes
