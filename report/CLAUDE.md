...
Sections in `report/`:
- `introduction.tex` — draft; opens with the project-pipeline overview figure (`fig:pipeline`, inline TikZ): sources (gamepad teleop + downloaded baseline) → **LeRobot** datasets/fine-tuning → **Isaac Sim** orchestrated inference (orchestrator sends GRASP/LIFT/PLACE prompts to the SmolVLA policy), plus the autonomous-data feedback loop
...
- `methodology.tex` — draft; Methods restructured — the dataset subsections are grouped under one `\subsection{Datasets}` (subsubsections: LightwheelAI Baseline, Teleoperation, Autonomous Data Generation, Dataset Composition, Monotask Fair Comparison); the order is 3.1 Policy, 3.2 Subtask Decomposition, 3.3 Orchestrator, 3.4 Datasets. The Teleoperation subsubsection notes the abandoned Meta Quest 3 hand-tracking attempt (SO-101 has 5 arm joints vs the 6 DOF a full hand pose needs)
