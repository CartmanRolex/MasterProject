...
| `compute_grasp_intent.py` | (no figure) Monotask grasp **intent** from the per-step `geometry_trace` (eval schema v4): distinct oranges **tried**/episode (fills the `tab:grasp_chain` monotask `pending` cells), single-orange **fixation** distribution, and median closest grip-axis approach. Tunable thresholds (`ENGAGE_M`, `MIN_ENGAGED_FRAMES`) so the definition changes with no eval rerun; degrades gracefully on pre-v4 checkpoints. Reads `flat_checkpoint.json` under `isaac-inference/results`; pure stdlib. |
