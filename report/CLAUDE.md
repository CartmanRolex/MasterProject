...
| `compute_obedience.py` | (no figure) Prints the **per-label grasp obedience** column of `tab:obedience` (the obedience section's consolidated prevalence-vs-obedience table). Reuses `plot_grasp_confusion.py`'s `confusion()` so the obeyed rate matches the figure's diagonal exactly, aggregated over both subtask models and scene states 0+1. Training-prevalence columns come from `extract_positional_prevalence.py`. Pure stdlib. |
