...
**Note (results rewrite).** The results section was consolidated around `tab:grasp_chain` and the obedience analysis. The old lift/place/recovery/failure-geometry tables and `fig:failure_modes` were removed, so `compute_recovery.py`, `compute_failure_modes.py`, `plot_failure_modes.py`, and the by-scene-state columns of `compute_place_success.py` are retained for reference but **no longer feed any report table/figure**.
