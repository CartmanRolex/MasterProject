...
palm-down, fingers forward), so the rotation readout shows the hand's true tilt
from horizontal regardless of how it is held at reset. A **Reset origin** button
re-anchors the position origin to the current pose and re-asserts the level
rotation reference (also done automatically when the Quest reconnects).
