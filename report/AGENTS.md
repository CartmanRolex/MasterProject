...
| `compute_place_success.py` | (no figure) Prints the place success rate (overall and by oranges-already-placed) for the `tab:place_success` table. Corrects the orchestrated runs' gripper-retraction confirmation gate (which logs placed-but-unretracted oranges as timeouts) by recounting an orange as placed when it ends in the plate / is never re-engaged; monotask rates use raw confirmation (no gate). Reads checkpoints under `isaac-inference/results`; pure stdlib. |
